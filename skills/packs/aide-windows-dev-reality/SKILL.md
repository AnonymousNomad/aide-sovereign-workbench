...
| node --test waits on leaked handles | All tests pass, runner never exits | FIX THE LEAK, DO NOT FORCE-EXIT. Close every handle in `after()` (`server.close()` + service `disconnect()`/destroy()) so the event loop drains and the runner exits with an honest code. `--test-force-exit` is a band-aid that masks the leak AND is dangerous on win32 — verified 8/27: it triggered a libuv `UV_HANDLE_CLOSING` native assert (run-arch.mjs had it REMOVED for this). Verified 8/27 in telegram-battery: proper cleanup → plain `node file.mjs` exits 0 on pass, 1 on real failure; a leaked-open `after()` (old `setImmediate(()=>process.exit(0))`) is the FAIL-SILENT defect that made the battery pass CI while never genuinely green. Prefer a `--timeout` as the honest safety net, not `--test-force-exit`. |
