...
try {
  ...
  for (const endpoint of ['/api/models/status', '/api/community', '/api/training/status', '/api/replays', '/api/workspace/tree', '/api/blueprint', '/api/academy', '/api/plugins', '/api/tasks']) { const result = await fetch(`http://127.0.0.1:4879${endpoint}`); assert.equal(result.status, 200, endpoint); }
