...
- **Pitfall 5 (learned 2026-09-01, R8-grade): NEVER author a >200-line service file via `editor.insert_line` chunked inserts.** The Cline editor's `insert_line` appends at the given line, so after 2-3 inserts on a non-empty file the brace nesting is silently corrupted. `node --check` accepts the corruption (it parses as a top-level block with the orphan `}`), but at runtime the factory returns `undefined` because the function body has been swallowed into a sibling block. **Symptom**: `svc = createWorktreeService({workspace:'...'})` returns `undefined` with no throw; `String(fn)` shows the function body ends with `}}` (two closing braces, the second is the orphan). **Fix**: delete the file, rewrite from the test/spec in ONE `fs.writeFileSync` call from a node script saved to `E:/pip_temp/`, then `node --check` AND the real `tests/arch/worktree-isolation.test.ts` (the test is the spec). The script-and-`fs.writeFileSync` pattern is in `aide-architect-editor-implementation` doctrine.
