...
if (!/^[a-z0-9-]+$/.test(label ?? '') || args.length === 0) throw new Error('label and node arguments required');
...
await fs.writeFile(new URL(`./phase1b-${label}.json`, import.meta.url), JSON.stringify({
  started, ended: new Date().toISOString(), args, pid: child.pid, freeMemoryBytes: os.freemem(), ...result, stdout, stderr
}, null, 2));
