...
try {
  ...
  assert.equal((await request('/api/blueprint')).body.layers.length, 5); assert.equal((await request('/api/providers')).body.providers.length, 1); assert.equal((await request('/api/artifacts')).response.status, 200);
