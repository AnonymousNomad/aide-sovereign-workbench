...
- Editor Core targeted gates: `editor/test-groups.mjs`, `session/test-store.mjs`, and `daemon/test-workspace-manager.mjs` pass. `scripts/acceptance-real.mjs` now has a bounded readiness wait and startup diagnostics, and passes its real workspace integration flow through LSP completion, task, Git, plugin, Academy, Blueprint, provider, artifact, and search paths.
...
- Next: run the remaining regression suite, commit/push the verified AIDE changes, then close the next Daily-Driver gaps: terminal/task UX, Git panel parity, LSP/DAP fixture completeness, and hot-exit recovery before desktop packaging.
