...
async function seededSystemPrompt(options) {
  ...
  const started = await agentFixture.startAgent(loop, 'tier probe', 'act');
  const system = loop.transcriptOf(started.session_id)[0].content;
  return { loop, started, system };
}

test('tier: default (no effectiveContextTokens) keeps the legacy full-credo prompt', async () => {
  const { system } = await seededSystemPrompt({});
...
test('tier: micro (<8192) swaps full credo for the 3-line operating layer but keeps the machine contract', async () => {
  const { system } = await seededSystemPrompt({ effectiveContextTokens: 2048 });
...
test('tier: full (>=8192) composes credo via the scaffold, not the hand-rolled loader', async () => {
  const { system } = await seededSystemPrompt({ effectiveContextTokens: 16384 });
...
test('tier: per-session override beats the loop-level default', async () => {
  ...
  const started = await agentFixture.startAgent(loop, 'override probe', 'act', null, { effectiveContextTokens: 16384 });
  const system = loop.transcriptOf(started.session_id)[0].content;
