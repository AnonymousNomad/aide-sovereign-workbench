...
try {
  ...
  const terminal = await request('/api/terminal/run', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ program: 'echo', args: ['terminal-ok'], approved: true }) }); assert.equal(terminal.body.stdout.trim(), 'terminal-ok');
  ...
  const gitStatus = await request('/api/git/status'); assert.equal(gitStatus.response.status, 200); assert.ok(gitStatus.body.files.some(file => file.path === 'README.md'), JSON.stringify(gitStatus.body));
  const gitDiff = await request('/api/git/diff?path=README.md'); assert.equal(gitDiff.response.status, 200); assert.match(gitDiff.body.diff, /patched/);
